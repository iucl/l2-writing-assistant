...
%MD: If space needed, cut: ; this makes the task quite different from translation in the general case
...
%MD: If space needed, cut: For the candidate generation, we later experiment with expanding to larger sets of data (section~\ref{sec:exp:large-pts}).
...
\marginpar{Is someone still working on this paragraph?}
...
%%%% CL THURS: myself is a little confused where we should use FIT or SIM for defining the equations above. I am voting fot FIT, because FIT meant to be the fitness of the context, which can involve more than one word. where SIM is defined first for just two words. 
